	...
	def _quantity_secondary_done_compute(self):
		for move in self:
			if move._get_move_lines():
				...
				quantity_done = move.product_id.uom_id._compute_quantity(move.quantity_done, move.product_id.secondary_uom_id, rounding_method='HALF-UP')
	...
	def secondary_qty_compute(self):
		for move_line in self:
			if move_line.product_id.is_secondary_uom:
				...
				move_line.update({'secondary_uom_id' : False})
				move_line.update({'secondary_quantity' : 0.0})
