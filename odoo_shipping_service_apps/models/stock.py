...
class StockMoveLine(models.Model):
    ...
    def manage_package_type(self):
        ...
        if res and (type(res)==dict):
            ...
            ctx={
                'no_description':
                        not(delivery_type in ['fedex', 'dhl', 'ups', 'auspost', 'canada_post', 'royal_mail'] and delivery_type or False ),
                'no_cover_amount' :
                        not(delivery_type in ['fedex', 'dhl', 'ups', 'usps', 'auspost', 'canada_post', 'royal_mail'] and delivery_type or False ),
                'no_edt_document' :
...
class ChooseDeliveryPackage(models.TransientModel):
    ...
    def onchange_delivery_packaging_id(self):
        ...
        if packaging_id:
            ...
            self.wkk_length = packaging_id.length
            self.cover_amount=packaging_id.get_cover_amount(self.cover_amount)
            _logger.info("===%r====%r====+%r====%r"%(self.height,self.width,self.wkk_length,self.cover_amount))
