...
class DeliveryCarrier(models.Model):
    ...
    @api.model
    def wk_get_carrier_settings(self,Fields):               # Method called by Every Delivery Carrier wiht Fields as Argument ,which should a list for return their credentials as Dictionary.
    ...
    def wk_get_product_package(self, line, packaging_id, partial_package=None):
        ...
        """
        dimension = dict(height=packaging_id.height, width=packaging_id.width,
                         length=packaging_id.length, packaging_id=packaging_id.id)
        result = list()
        line_price = 0
        if partial_package:
        ...
        max_qty, max_weight = int(packaging_id.qty) and int(
            packaging_id.qty) or 1, packaging_id.max_weight and packaging_id.max_weight or 1
        ...
        if pickings:
            if not pickings.package_ids:
                ...
                package_ids = pickings.package_ids.filtered(
                    lambda package_id: not package_id.packaging_id)
                if len(package_ids):
