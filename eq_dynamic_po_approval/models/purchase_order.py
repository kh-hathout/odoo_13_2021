...
class purchase_order(models.Model):
    ...
    def do_purchase_approve(self):
        for order in self:
            if order.state == 'to approve':
                if order.next_approved_by_user:
                    if not self.env.is_admin() and self.env.user not in order.next_approved_by_user:
                        raise Warning(_("You don't have rights to approved order."))
                if order.next_approved_by_group:
                    if not self.env.is_admin() and order.next_approved_by_group not in self.env.user.groups_id:
                        raise Warning(_("You don't have rights to approved order."))
    ...
    def update_next_approval(self,next_approval_line_id):
        ...
        update_vals = {'state':'to approve','next_approval_line_id':next_approval_line_id.id}
        update_vals['next_approved_by_user'] = [(6,0,users_ids.ids)] if users_ids else False
        ...
        if user_ids:
            msg = _("Dear %s, <br/> Please approve purchase order %s.") % (', '.join([str(user.name) for user in user_ids]),self.name)
    ...
    def find_order_approval(self,approval_line_id=None):
        ...
        if self.currency_id.id != company_id.currency_id.id:
            main_curreny_total = self.currency_id._convert(self.amount_total, company_id.currency_id, company_id, self.date_order or fields.Date.today())

        find_approval_id = po_approval_ids.filtered(lambda l: main_curreny_total >= l.min_amount and main_curreny_total <= l.max_amount)
        if find_approval_id:
            approval_line_ids = find_approval_id.approval_line_ids
            if approval_line_id and approval_line_id.config_approval_id.id == find_approval_id.id and find_approval_id.approval_line_ids.filtered(lambda l: l.sequence == approval_line_id.sequence):
                approval_line_ids = find_approval_id.approval_line_ids.filtered(lambda l: l.sequence > approval_line_id.sequence)
